...
export const BranchOfficeGridItem = ({ branch }: Props) => {
  ...
  return (
    <Link
      ...
      key={branch.name}
      className=" relative  flex flex-col rounded-md hover:scale-105 transition-all"
    >
      {branch.officeType === "BARBERSHOP" ? (
        <span className=" bg-secondary text-xs px-2 p-1 top-1 left-0 absolute rounded-r-md">
          Barbería
        </span>
      ) : (
        <span className="bg-[#472020] text-xs px-2 p-1  top-1 left-0 absolute rounded-r-md">
          Salón
        </span>
      )}
      <Image
