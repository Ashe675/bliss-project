...
const EmployeeProfile = async ({ params }: EmployeeProfileProps) => {
  ...
  return (
    <main className='m-6 lg:m-10  2xl:grid 2xl:grid-flow-col '>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {data?.posts?.map((post: Post) => (
                <div key={post.id} className="bg-gradient-to-r from-red-950 border-red-950 border via-orange-800/80 to-primary/80 rounded-lg shadow-md hover:shadow-lg overflow-hidden              transition-transform duration-300 hover:scale-105 hover:bg-right animate-gradient">
                    ...
                    <div className="p-4">
                        ...
                        <p className="text-sm text-gray-200">{new Date(post.createdAt).toLocaleDateString('es-ES', {day: '2-digit', month: 'long', year: 'numeric'})}</p>
                    </div>
                </div>
            ))}
        </div>
      ...
      <section>
        ...
        <div className="space-y-4">
          {data?.reviews?.map((review: Review) => (
            <div key={review.id} className="p-4 bg-gradient-to-r from-red-900/80 border-red-950 border via-red-950 to-primary rounded-lg mb-2 shadow-sm space-y-2          transition-transform duration-300 hover:scale-105 hover:bg-right animate-gradient">
      ...
      <div className="flex justify-center grid-cols-3 mx-40  pb-40 mb-40">
